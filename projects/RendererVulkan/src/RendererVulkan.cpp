...
namespace gg
{
		...
		, camera{ std::make_shared<Camera>(rs.inputManager) }
	{
		...
		{ /* ImGUI */
			...
			DebugUI_VulkanCreateInfo debugUI_info{};
			debugUI_info.device = device;
			debugUI_info.instance = instance;
			debugUI_info.graphicsQueueFamily = indices.graphicsFamily.value();
			debugUI_info.graphicsQueue = graphicsQueue;
			debugUI_info.physicalDevice = physicalDevice;
			debugUI_info.renderPass = renderPass;
			debugUI_info.windowHandleSDL = windowHandleSDL;
			debugUI_info.renderer = this;

			debugUI = std::make_unique<DebugUI_Vulkan>(debugUI_info);
		}
	}
	...
	{
		std::shared_ptr<ShaderProgram> shader = LoadShader(skyboxShaderName);
		glm::vec3 position{};
		std::shared_ptr<ModelVulkan> model = std::make_shared<ModelVulkan>(skyboxModelName, shader, position, device);

		CreateVertexBuffer(model);
		CreateGraphicsPipeline(model);

		SkyboxTextures skyboxTextures{ GetSkyboxTexturePaths(name) };
		std::shared_ptr<TextureVulkan> skybox = std::make_shared<TextureVulkan>(skyboxTextures, device);
		model->textures.push_back(skybox);
